...
function ResultView({ pet, result, save, discover, fieldGuideUnlocked }: { pet: Pet; result: Result; save: () => void; discover: () => void; fieldGuideUnlocked: boolean }) {
  return <main className="result-page">
    ...
    {fieldGuideUnlocked ? <section className="care-plan"><p className="eyebrow dark">Complete Field Guide · Unlocked</p><h2>A small care plan for the next week</h2><div>{result.recommendations.map((r, i) => <article key={r.title}><span>0{i + 1}</span><h3>{r.title}</h3><p><strong>Try:</strong> {r.try}</p><p><strong>Look for:</strong> {r.look}</p><small>Pause if tension, avoidance, escalating frustration, or prolonged recovery appears.</small></article>)}</div></section>
      : <section className="field-guide-lock"><div><p className="eyebrow dark">Complete Field Guide</p><h2>Turn the map into a week of practical care.</h2><p>Your free Snapshot stays available. A one-time Field Guide purchase unlocks the personalized seven-day plan and one follow-up retest for this pet.</p></div><div><strong>$5.99 <small>USD · one time</small></strong><Link className="plan-button" href="/checkout">Unlock with PayPal <span>↗</span></Link><small>Sandbox checkout · No real funds move during testing</small></div></section>}
