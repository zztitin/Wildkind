...
type Result = { scores: Record<DomainCode, number | null>; coverage: Record<DomainCode, string>; archetype: string; second?: string; essence: string; healthFlag: boolean; profileComplete?: boolean; recommendations: Array<{ title: string; try: string; look: string }> };
...
function Header({ screen, setScreen, signedIn, initials }: { screen: Screen; setScreen: (s: Screen) => void; signedIn: boolean; initials: string }) {
  ...
  return (
    <header className="site-header">
      ...
      ) : <div className="header-public"><span className="header-note">Behavior, carefully observed.</span><Link href="/resources/pet-behavior-observation-checklist">Free checklist</Link><Link href="/sample-field-guide">Sample report</Link><Link href="/methodology">Methodology</Link><Link href="/about">About</Link><Link href="/pricing">Pricing</Link></div>}
...
function computeResult(answers: Answers): Result {
  ...
  const profileComplete = scoreList.every(s => s !== null);
  const ranked = archetypes.map(a => ({ a, distance: profileComplete ? Math.sqrt(scoreList.reduce((sum, s, i) => sum + Math.pow((s as number) - a[1][i], 2), 0)) : 999 })).sort((a,b) => a.distance - b.distance);
  ...
  return { scores, coverage, archetype: first.a[0], second: profileComplete && blended ? second.a[0] : undefined, essence: first.a[2], healthFlag, profileComplete, recommendations };
}

function ResultView({ pet, result, save, discover, fieldGuideUnlocked }: { pet: Pet; result: Result; save: () => void; discover: () => void; fieldGuideUnlocked: boolean }) {
  const profileComplete = result.profileComplete ?? domains.every(d => result.scores[d.code] !== null);
  return <main className="result-page">
    ...
    {!result.healthFlag && profileComplete && <section className="archetype"><p className="eyebrow dark">Provisional narrative match</p><div className="archetype-main"><Mark /><div><span>{result.second ? "A blended pattern" : "Nearest field-guide pattern"}</span><h2>{result.archetype}</h2>{result.second && <h3>with {result.second} qualities</h3>}<p>{result.essence}. The archetype is a storytelling layer, not a fixed biological type.</p></div></div></section>}
    {!result.healthFlag && !profileComplete && <section className="limitation paper"><strong>Archetype not calculated</strong><p>At least one coordinate has insufficient observation. Complete all five dimension scores before comparing this Snapshot with a narrative prototype.</p></section>}
    {fieldGuideUnlocked ? <section className="care-plan"><p className="eyebrow dark">Complete Field Guide · Unlocked</p><h2>A small care plan for the next week</h2><div>{result.recommendations.map((r, i) => <article key={r.title}><span>0{i + 1}</span><h3>{r.title}</h3><p><strong>Try:</strong> {r.try}</p><p><strong>Look for:</strong> {r.look}</p><small>Pause if tension, avoidance, escalating frustration, or prolonged recovery appears.</small></article>)}</div></section>
      : <section className="field-guide-lock"><div><p className="eyebrow dark">Complete Field Guide</p><h2>Turn the map into a week of practical care.</h2><p>Your free Snapshot stays available. A one-time Field Guide purchase unlocks the personalized seven-day plan and one follow-up retest for this pet.</p></div><div><strong>$5.99 <small>USD · one time</small></strong><Link className="plan-button" href="/checkout">Unlock with PayPal <span>↗</span></Link><small>Secure one-time payment · No subscription</small></div></section>}
...
export function WildKindApp({ initialUser }: { initialUser: { name: string | null; email: string } | null }) {
  const [screen, setScreen] = useState<Screen>("home"); const [pet, setPet] = useState<Pet>(defaultPet); const [answers, setAnswers] = useState<Answers>({}); const [index, setIndex] = useState(0); const [result, setResult] = useState<Result>(); const signedIn = Boolean(initialUser); const [fieldGuideUnlocked, setFieldGuideUnlocked] = useState(false); const [connections, setConnections] = useState<string[]>([]); const [messages, setMessages] = useState<Record<string,string[]>>({}); const [blocked, setBlocked] = useState<string[]>([]); const [reporting, setReporting] = useState(false); const [loaded, setLoaded] = useState(false);
  useEffect(() => { fetch("/api/state").then(r=>r.ok?r.json():{}).then((s:SavedState)=>{ const draft=sessionStorage.getItem("wk-draft"); if(draft)setAnswers(JSON.parse(draft)); if(s.pet)setPet(s.pet); if(s.result)setResult(s.result); if(s.connections)setConnections(s.connections); if(s.messages)setMessages(s.messages); if(s.blocked)setBlocked(s.blocked); if(s.result && new URLSearchParams(window.location.search).get("open") === "field-guide") setScreen("result"); }).finally(()=>setLoaded(true)); }, []);
  useEffect(() => { if (signedIn) void fetch("/api/paypal/entitlement").then(response => response.ok ? response.json() : { fieldGuide:false }).then((entitlement:{ fieldGuide?:boolean }) => setFieldGuideUnlocked(Boolean(entitlement.fieldGuide))); }, [signedIn]);
  ...
  return <div className={`site-shell screen-${screen}`}><Header screen={screen} setScreen={setScreen} signedIn={signedIn} initials={initials} />
    ...
    {screen === "result" && result && <ResultView pet={pet} result={result} save={save} fieldGuideUnlocked={fieldGuideUnlocked} discover={() => { if (!signedIn) { saveBeforeRegistration(); return; } setScreen("discover"); }} />}
    ...
    <footer><div className="brand"><Mark small /><span>WILDKIND</span></div><p>Warm science for the individual animal.</p><nav><Link href="/resources/pet-behavior-observation-checklist">Free checklist</Link><Link href="/pricing">Pricing</Link><Link href="/sample-field-guide">Sample report</Link><Link href="/about">About</Link><button onClick={() => setReporting(true)}>Safety</button><button onClick={() => setScreen(signedIn ? "privacy" : "home")}>Privacy</button><Link href="/methodology">Methodology</Link></nav><small>© 2026 WildKind · Snapshot v0.1 · Not veterinary advice</small></footer>
  </div>;
}
